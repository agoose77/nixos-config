  ...
  wayland.windowManager.hyprland = {
    ...
    settings = {
      ...
      bind = [
        ...
        "$mod, return, exec, ${uwsm-app (lib.getExe pkgs.kitty)}"
        "$mod SHIFT, return, exec, ${uwsm-app (lib.getExe pkgs.firefox)}"
        "$mod SHIFT, Q, killactive, g"
        "$mod, SPACE, exec, ${uwsm-app (lib.getExe pkgs.fuzzel)} --show drun"
        "$mod SHIFT, SPACE, exec, ${uwsm-app (lib.getExe pkgs.fuzzel)} --show run"
        "$mod, M, exit, "
